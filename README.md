...
MusicBee の `設定` → `プラグイン` → `AutoLyrics` → `設定` から以下を調整できます：
- **歌詞ソース**: 各プロバイダの有効 / 無効の切り替え
- **曲名類似度 (%)**: 照合時の許容類似度（デフォルト: 85%）
- **アーティスト類似度 (%)**: 照合時の許容類似度（デフォルト: 85%）
- **同期歌詞(LRC)を優先**: タイムタグ付き同期歌詞が利用可能な場合に優先保存
